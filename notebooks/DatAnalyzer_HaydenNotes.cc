// Header file imported, contain declrations for functions and classes
#include "DatAnalyzer.hh"

// Similar to a constant in Python
#define DEFAULT_FOR_EMPTY_CH 0

// Import the entire C++ standard library namespcae (vector, cout etc...)
using namespace std;

// Constructor for the DatAnalyzer class, like the __init__ method in python
DatAnalyzer::DatAnalyzer(int numChannels, int numTimes, int numSamples, int res, float scale, int numFsamples) : // : is called the intializer list
        // not function calls they are initializations of variables
        ...
        file(0), tree(0) // initializes file and tree with zero
{
}

// This is the destructor for C++. Used when the class is destroyed. 
// Happens when Program exiting
// also for example when an object is made inside of a function and the function exits, the destructor will be called
// Can also happend when creaeting an object using "new" and needs to be deleted using delete
// Example:
//DatAnalyzer* analyzer = new DatAnalyzer(1, 2, 3, 4, 5.0, 6);
// delete analyzer; // analyzer is destroyed here
DatAnalyzer::~DatAnalyzer() //destructor signified by the ~
{
