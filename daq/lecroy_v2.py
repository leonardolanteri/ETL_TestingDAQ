...
class RunLecroy:
    ...
    def vertical_setup(self, channel: int, start: float, stop: float) -> None:
        """
        ...
        """   
        volts_div = (stop - start) / self.num_vertical_divs
        print(f"Setting channel {channel} to {volts_div} V/div")

        self.lecroy.write(f"C{channel}:COUPLING D50")
        self.lecroy.write(f"C{channel}:VOLT_DIV {volts_div}V")
        self.lecroy.write(f"C{channel}:OFFSET {0}V")

    def horizontal_setup(self, start:float, stop: float) -> None:
        """
        ...
        """
        time_div = (stop-start)/self.num_horizontal_divs
