...
def setup_etl_telescope(etl_telescope: ETL_Telescope, run_config: RunConfig, telescope_config: TelescopeConfig):
...
class RunKcuStream:
    ...
    """
    def __init__(self, kcu_ipadress: str, project_dir: Path):
        self.kcu_ip_address = kcu_ipadress
        self.project_dir = proj_dir
        self.run_number_path  = project_dir / Path('daq/next_run_number.txt')
        self.is_scope_acquiring_path = project_dir / Path('daq/is_scope_acquiring.txt')
    ...
    def etroc_daq_command(self) -> list[str]:
        ...
        return ['/usr/bin/python3', str(etroc_daq_script), 
            ...
            '--rb', '0', # do comma seperated for each rb
            '--run', str(self.run_number), 
            '--lock', str(self.is_scope_acquiring_path)] # lock waits for scope to be ready

    def setup(self) -> None:
        etroc_daq_process = subprocess.Popen(['/usr/bin/python3', 'daq/test.py']) #self.etroc_daq_command
        #etroc_daq.wait()
...
setup_etl_telescope(etl_telescope, tb_config.run_config, tb_config.telescope_config)
...
with LecroyController(scope_config.ip_address, active_channels=active_channels) as lecroy:
    ...
    with RunKcuStream(tb_config) as kcu_stream:
        kcu_stream.setup()
        time.sleep(3) #allow for everything to get setup, i dont like this logic, could instead let kcu stream know when all are ready!
